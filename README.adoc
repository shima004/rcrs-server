...
The cross-platform Java launcher does not require Bash. Open a terminal window,
navigate to the ```rcrs-server``` root directory and execute one of the following:

```bash

$ java -jar jars/rcrs-server-launcher.jar precompute

$ java -jar jars/rcrs-server-launcher.jar comprun
```

The launcher accepts the same map, config, log, team, GUI, and JLog options as
the shell scripts. Each process heap limit can be overridden by repeating the
`--memory <name>=<size>` option. For example:

```bash

$ java -jar jars/rcrs-server-launcher.jar comprun --memory kernel=4g --memory traffic=2g
```

Run the following command to see all options and supported process names:

```bash

$ java -jar jars/rcrs-server-launcher.jar --help
```

The Bash launcher remains available on systems that support it:
...
Release images are published to the GitHub Container Registry. The default
container command starts a headless competition run:

```bash

$ docker run --rm -p 27931:27931 ghcr.io/shima004/rcrs-server:latest
```

Launcher arguments can be supplied after the image name:

```bash

$ docker run --rm -p 27931:27931 \
    -v rcrs-logs:/opt/rcrs-server/logs \
    ghcr.io/shima004/rcrs-server:latest \
    comprun --nogui --memory kernel=4g
```

The image is built with OpenJDK 21 whenever a GitHub Release is published.
