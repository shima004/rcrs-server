...
if (( $TYPE == 4)); then
    echo "Running: /bin/sh \"precompute.sh\" 1 1 1 1 1 1 $HOST 2>&1 | tee $LOGFILE"
   /bin/sh "precompute.sh" 1 1 1 1 1 1 $HOST 2>&1 | tee $LOGFILE

fi
# firebrigade
if (( $TYPE == 1)); then
    echo "Running: /bin/sh \"precompute.sh\" 1 0 0 0 0 0 $HOST 2>&1 | tee $LOGFILE"
    /bin/sh "precompute.sh" 1 1 0 0 0 0 $HOST 2>&1 | tee $LOGFILE
#    /bin/sh "precompute.sh" 1 1 1 1 1 1 $HOST 2>&1 | tee $LOGFILE
...
if (( $TYPE == 2)); then
    ...
    /bin/sh "precompute.sh" 0 0 1 1 0 0 $HOST 2>&1 | tee $LOGFILE
fi

# ambulance
if (( $TYPE == 3)); then
    echo "Running: /bin/sh \"precompute.sh\" 0 0 0 0 1 0 $HOST 2>&1 | tee $LOGFILE"
    /bin/sh "precompute.sh" 0 0 0 0 1 1 $HOST 2>&1 | tee $LOGFILE
fi
