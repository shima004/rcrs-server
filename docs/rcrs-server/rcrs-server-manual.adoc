...
After running `./gradlew completeBuild`, both modes can be started without a
POSIX shell by using the cross-platform Java launcher from the project root:
...
Run `java -jar jars/rcrs-server-launcher.jar --help` for all available options.
The maximum heap size of each process can be changed by repeating
`--memory <name>=<size>`, for example
`--memory kernel=4g --memory traffic=2g`.
